...
;; TODO: check for lost reader conditionals

(defprotocol Schema
  (spec [this]
    "A spec is a record of some type that expresses the structure of this schema
     in a decomposed way. See schema.spec.core.  TODO: why?")
  (explain [this]
  ...
  (spec [this]
    (let [pre (instance-precondition this this)]
      (if-let [class-schema (utils/class-schema this)]
        (spec (constrained class-schema pre))
        (leaf/leaf-spec pre))))
  (explain [this]
...
(clojure.core/defn either
  "A value that must satisfy at least one schema in schemas.
   ...
   NOTE: either is slow and gives bad error messages.  Please consider using
   `conditional` and friends instead, they will make your life much easier.
   (more efficient, better error messages, works with coercion).

   TODO CHANGELOG: no longer works with coercion"
  [& schemas]
  (Either. schemas))
...
(clojure.core/defrecord Constrained [schema pre]
  Schema
  (spec [this] (variant/variant-spec pre [{:schema schema}]))
  (explain [this] (cons 'constrained (explain schema))))
...
(clojure.core/defn constrained
  ...
  [schema precondition]
  (Constrained. schema precondition))
...
(clojure.core/defn both
  "A value that must satisfy every schema in schemas.
     TODO: CHANGELOG: DEPRECATED, prefer 'guarded' instead'
...
;; TODO: how do we do clean final error?
...
(defmacro record
  ;; TODO: note in changelog possible breakage.
  ;; TODO: cljs
